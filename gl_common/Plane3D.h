...
public:
    ...
    GLPlane3D(float center_x, float center_y, float center_z, float width, float height, int val, int rows = 10, int cols = 10 );
    ~GLPlane3D();
...
protected:
    ...
    */
    void make_box(float center_x, float center_y, float center_z, float width, float height, int val, int rows, int cols, vector<Vertex>& vp, vector<Vertex>& nv );
